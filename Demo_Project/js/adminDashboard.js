...
function addOnProject(EmployeeID) {
	$.ajax({
				url : "/Demo/components/dao/projectDao.cfc?method=getAllActiveProject&queryformat=column",
				dataType : "json",
				type : "POST",
				success : function(response) {
					$("#projectList").html("");
					if (response) {
						for (i = 0; i < response.DATA.TITLE.length; i++) {
							var pid = response.DATA.PROJECTID[i];
							$("#projectList").append('<li class="list-group-item" onClick=assignProject('+pid+','+EmployeeID+');>'+response.DATA.TITLE[i]+'</li>');
						}
						$("#addOnProject").modal('show');
...
function showProjectDetails(title, descreption) {
	$("#showTitle").html(title);
	$("#showDesc").html(descreption);
	$("#showProjectDetails").modal('show');

}

function showEmployeeOnProject(projectId) {
	$
			.ajax({
				url : "/Demo/components/dao/employeeDao.cfc?method=getEmployeeOnProject&queryformat=column&projectId="
						+ projectId,
				dataType : "json",
				type : "POST",
				success : function(response) {
						...
						for (i = 0; i < response.DATA.EMPLOYEEID.length; i++) {
							$("#resProject").append(
									'<li class="list-group-item" >'
											+ response.DATA.EMPLOYEENAME[i]
											+ '   |   '
											+ response.DATA.EMPLOYEEID[i]
											+ '</li>');
						}
						$("#showEmployeeOnProject").modal('show');
...
function editProject(projectId) {
	$
			.ajax({
				url : "/Demo/components/dao/projectDao.cfc?method=getProjectById&queryformat=column&projectId="
						+ projectId,
				dataType : "json",
				type : "POST",
				success : function(response) {
					...
					if (response) {
						$("#projectId").html(
								"<span>ProjectID:  </span>" + projectId);
						$("#edit_projectId").val(projectId);
						$("#edit_project_title").val(response.DATA.TITLE[0]);
						$("#edit_project_descreption").val(
								response.DATA.DESCREPTION[0]);
						$("#edit_team_leader_hidden").val(
								response.DATA.TEAMLEADER[0]);
						$("#edit_team_leader").val(
								response.DATA.TEAMLEADERNAME[0]);
