...
<cfcomponent>
	...
	<cffunction name="getAllEmployee" returnType="query">
		...
		<cftry>
			<cfquery name="rsDetails">
				...
				Proj.[Descreption]
				FROM
				(([dbOrganisation].[Employee].[Employee] AS Emp
				LEFT OUTER JOIN [dbOrganisation].[Employee].[Emp_Project] AS Emp_Proj ON Emp.EmployeeID=Emp_Proj.EmployeeID)
				LEFT OUTER JOIN [dbOrganisation].[Employee].[Project] AS Proj ON Proj.ProjectID=Emp_Proj.ProjectID)
	...
	<!---method for getting  employee project details  --->

	<cffunction name="getEmployeeProjectDetails" returnType="query">
		<cfset var rsDetails=""/>
		<cftry>
			<cfquery name="rsDetails">
				SELECT
				...
				Proj.[Title],
				Proj.[Descreption],
				CONCAT( EmpTeam.[FirstName], ' ',EmpTeam.[MiddleName],' ',EmpTeam.[LastName]) AS TeamLeaderName
				FROM
				((([dbOrganisation].[Employee].[Employee] AS Emp
				LEFT OUTER JOIN [dbOrganisation].[Employee].[Emp_Project] AS Emp_Proj ON Emp.EmployeeID=Emp_Proj.EmployeeID)
				LEFT OUTER JOIN [dbOrganisation].[Employee].[Project] AS Proj ON Proj.ProjectID=Emp_Proj.ProjectID)
				LEFT OUTER JOIN [dbOrganisation].[Employee].[Employee] AS EmpTeam ON Proj.[TeamLeader]=EmpTeam.EmployeeID)
				WHERE Emp.EmployeeId = <cfqueryparam value ='#session.stLoggedInUser.employeeId#'CFSQLType ='CF_SQL_INTEGER'>
			</cfquery>
			<cfcatch type="database">
				<p>
					<strong>
	...
	<cffunction name="getEmployeeOnProject" access = "remote" returnType="query" returnFormat="json">
		...
		<cftry>
			<cfquery name="rsDetails">
				...
				Proj.Title
				FROM
				(([dbOrganisation].[Employee].[Project] AS Proj
				INNER JOIN [dbOrganisation].[Employee].[Emp_Project] AS Emp_Proj ON Proj.ProjectID=Emp_Proj.ProjectID)
				INNER JOIN [dbOrganisation].[Employee].[Employee] AS Emp ON Emp.EmployeeID=Emp_Proj.EmployeeID)
				WHERE Proj.ProjectID =<cfqueryparam value="#arguments.projectId#" cfsqltype="CF_SQL_INTEGER">
			</cfquery>
			<cfcatch type="database">
				<p>
					<strong>
	...
	<!---method for getting details of a project from cfm   --->

	<cffunction name="getEmployeeOnProjectById" returnType="query">
		<cfargument name="projectId" type="string" required="true">
		<cfset var rsDetails=""/>
		<cftry>
			<cfquery name="rsDetails">
